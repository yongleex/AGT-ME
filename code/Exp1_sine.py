...
def exp():
    ...
    # Step 3. gamma estimation from AGT-ME
    gamma_agt_me = -1 / np.mean(np.log((y_uint8_d.astype(np.float) + 0.5) / 256))
    print("Synthetic gamma amount:" + str(gamma))
    print("Estimated gamma amount (AGT-ME):" + str(1 / gamma_agt_me))

    gamma_cab = -np.log(2.0) / np.log(np.mean((y_uint8_d.astype(np.float) + 0.5) / 256))
    print("Synthetic gamma amount:" + str(gamma))
    print("Estimated gamma amount (CAB):" + str(1 / gamma_cab))
    ...
    for k, gamma in enumerate(gamma_list):
        img = (gamma_trans(y_uint8_d, 1 / gamma)[np.newaxis, :]).astype(np.uint8)
        BIGC_cost[k] = get_bi_coherence_cost(img)
    gamma_BIGC = gamma_list[np.argmin(BIGC_cost)]
    print("Estimated gamma amount (BIGC):" + str(gamma_BIGC))
    ...
    plt.plot(n, (y + 0.5) / 256, "b--", label="y(n)")
    plt.plot(n, (y_uint8_d + 0.5) / 256, "k-", label="$y^{1.5}(n)$")
    plt.xlim([0, 120])
    plt.ylim([0, 1])
    ...
    foo_fig = plt.gcf()
    foo_fig.savefig("figs/Fig2a.eps", format='eps', dpi=1200)
    ...
    plt.subplots_adjust(bottom=0.14)
    plt.plot(n, (y + 0.5) / 256, "b--", label="y(n)")
    plt.plot(n, ((y_uint8_d + 0.5) / 256) ** (1 / gamma_BIGC), "r-.", label="BIGC")
    plt.plot(n, ((y_uint8_d + 0.5) / 256) ** gamma_cab, "r:", label="CAB")
    plt.plot(n, ((y_uint8_d + 0.5) / 256) ** gamma_agt_me, "b-", label="AGT-ME")
    plt.xlim([0, 120])
    plt.ylim([0, 1])
    plt.xlabel("n", fontsize=16)
    plt.ylabel("y", fontsize=16)
    ...
    plt.legend(fontsize=16)
    foo_fig = plt.gcf()
    foo_fig.savefig("figs/Fig2b.eps", format='eps', dpi=1200)
    plt.title("Restore curve")

    # Figure 1c. BIGC loss curve
    plt.figure()
    plt.subplots_adjust(bottom=0.14)
    plt.plot(gamma_list, np.log(BIGC_cost), "r-.", label="BIGC")
    plt.xlim([0, 3])
    ...
    plt.legend(fontsize=16)
    foo_fig = plt.gcf()
    foo_fig.savefig("figs/Fig2c.eps", format='eps', dpi=1200)
    plt.title("BIGC Loss")
    ...
    plt.plot(gamma_list, entropy_smooth_cost, "b-", label="$J(\gamma)$ from Eq.(7)")
    plt.xlim([0.5, 3])
    plt.ylim([-5.3, -4.3])
    plt.xlabel("Reciprocal of correction gamma", fontsize=16)
    plt.ylabel("Loss", fontsize=16)
    ...
    plt.legend(fontsize=16)
    foo_fig = plt.gcf()
    foo_fig.savefig("figs/Fig2d.eps", format='eps', dpi=1200)
    plt.title("AGT-ME Loss")
