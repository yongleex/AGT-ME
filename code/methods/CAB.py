...
def correct_average_brightness(image, mask=None, normalize=False):
    """
    ...
    if normalize:  # normalization
        ...
        img = (255 * (img - np.min(img[:])) / (np.max(img[:]) - np.min(img[:]) + 0.001)).astype(np.float)
...
def simple_example():
    ...
    gamma, output = correct_average_brightness(image)
    end_time = time.time()
