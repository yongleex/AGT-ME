...
def predict_negative_entropy_cost(img, gamma):
    ...
    for i in range(256):
        p[i] = np.sum(img == i) / np.size(img)  # Get the histogram frequency with intensity level i
        cost = cost + p[i] * np.log(1e-10 + p[i] * ((i + 0.5) / 256.0) ** (
                1 - gamma) / gamma)  # Get the negative entropy loss after gamma distortion
    return cost
...
def exp():
    ...
    # Step 3. get the gamma value with our AGT-ME
    gamma_ours = -1 / np.mean(np.log((y_uint8_d.astype(np.float) + 0.5) / 256))
    print("Synthetic gamma amount:" + str(gamma))
    print("Estimated gamma amount (AGT-ME):" + str(1 / gamma_ours))
    ...
    foo_fig = plt.gcf()
    foo_fig.savefig("Fig1a.eps", format='eps', dpi=1200)
    ...
    plt.plot(n, ((y_uint8_d + 0.5) / 256) ** gamma_ours, "b-", label="AGT-ME")
    plt.xlim([0, 120])
    plt.ylim([0, 1])
    plt.xlabel("n", fontsize=16)
    plt.ylabel("y", fontsize=16)
    ...
    plt.legend(fontsize=16)
    foo_fig = plt.gcf()
    foo_fig.savefig("Fig1b.eps", format='eps', dpi=1200)
    ...
    plt.xlim([0, 3])
    plt.xlabel("$1/\gamma$", fontsize=16)
    plt.ylabel("Loss", fontsize=16)
    plt.tick_params(labelsize=16)
    plt.yticks([])
    plt.legend(fontsize=16)
    foo_fig = plt.gcf()
    foo_fig.savefig("Fig1c.eps", format='eps', dpi=1200)
    plt.title("BIGC Loss")
    ...
    plt.plot(gamma_list, entropy_smooth_cost, "b-", label="$J(\gamma)$")
    plt.xlim([0.5, 3])
    plt.ylim([-5.3, -4.3])
    plt.xlabel("$1/\gamma$", fontsize=16)
    plt.ylabel("Loss", fontsize=16)
    ...
    plt.legend(fontsize=16)
    foo_fig = plt.gcf()
    foo_fig.savefig("Fig1d.eps", format='eps', dpi=1200)
