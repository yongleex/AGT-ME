...
MAX_NUMBER = 2  # -1: all image employed; 2: accelerate for you to test it
...
def exp():
    ...
    plt.xlim([-0.2, 3.2])
    plt.ylim([-0.2, 3.2])
    plt.xlabel("actual gamma $\gamma_a$", fontsize=16)
    plt.ylabel("estimated gamma $\gamma_e$", fontsize=16)
    plt.tick_params(labelsize=16)
    # plt.legend(fontsize=20)
    foo_fig = plt.gcf()
    foo_fig.savefig("Fig3a.eps", format='eps', dpi=1200)
    ...
    temp[temp > 0.25] = np.NaN  # special treatment for BIGC outliers
    rmse_bigc = np.sqrt(np.nanmean(temp, axis=1))
    rmse_agt[rmse_agt < 1e-3] = 1e-3
    rmse_bigc[rmse_bigc < 1e-3] = 1e-3
    print("mean rmse:", np.mean(rmse_agt[:]), "(AGT-ME) and ", np.mean(rmse_bigc[:]), "(BIGC)")
    ...
    foo_fig = plt.gcf()
    foo_fig.savefig("Fig3b.eps", format='eps', dpi=1200)
